...
import { generateReferralLink, updateUserBalance } from '@/data';
...
const SPIN_REWARDS = [
	{
		...
		color: '#607D8B',
		actualReward: 100,
		weight: 40, // 40% chance - most common
		displayValue: '100',
	},
	{
		id: 2,
...
// Floating Spin Button Component
const FloatingSpinButton = ({ spinsAvailable, onClick }) => {
	return (
		<motion.div
		...
		>
			<motion.button
				onClick={onClick}
				className='relative w-16 h-16 bg-gradient-to-r from-purple-600 to-pink-600 rounded-full shadow-2xl flex items-center justify-center'
...
const SpinWheel = ({ isSpinning, selectedReward, onSpinStart, onSpinComplete }) => {
	...
	return (
		<div className='flex flex-col items-center gap-6'>
			{/* Wheel Container with Indicator */}
			<div className='relative w-80 h-80'>
				...
				>
					{SPIN_REWARDS.map((reward, index) => {
						...
						const radius = 100; // Distance from center for text
						const textAngleRad = (midAngle - 90) * (Math.PI / 180); // -90 to start from top
						const x = 160 + radius * Math.cos(textAngleRad);
						const y = 160 + radius * Math.sin(textAngleRad);
					...
					{/* Center circle */}
					<div className='absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-20 h-20 bg-gradient-to-r from-yellow-400 to-yellow-600 rounded-full flex items-center justify-center shadow-lg border-4 border-white z-20'>
...
const SpinModal = ({
	...
	const handleSpin = async () => {
		if ((currentUser.freeSpins || 0) <= 0) {
			toast({
				...
				description: 'Refer more friends to earn free spins!',
				variant: 'destructive',
	...
	const handleSpinComplete = async (detectedReward) => {
		try {
			...
			// Update user balance and reduce free spins
			await updateUserBalance(currentUser.id, finalReward.actualReward);
	...
	return (
		<div className='fixed inset-0 flex items-center justify-center bg-black/90 backdrop-blur-sm z-50 p-4'>
			<motion.div
				...
				className='bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-600/50 text-white w-full max-w-lg p-6 rounded-3xl shadow-2xl relative max-h-[90vh] overflow-y-auto'
			>
				{/* Close Button */}
				<button
				...
				<AnimatePresence>
					{showResult && spinResult && (
						...
						>
							<div className='bg-gradient-to-r from-green-600/30 to-emerald-600/30 border-2 border-green-500/60 rounded-2xl p-6 shadow-xl'>
								<motion.div
									initial={{ scale: 0 }}
									animate={{ scale: 1 }}
									transition={{ delay: 0.2, type: 'spring', stiffness: 200 }}
								>
									<h3 className='text-2xl font-bold text-green-400 mb-3 flex items-center justify-center gap-2'>
										🎉 Congratulations!
									</h3>
								</motion.div>
								<motion.div
				...
				{/* How to earn spins */}
				<div className='bg-gradient-to-r from-orange-600/10 to-orange-800/10 backdrop-blur-sm border border-orange-500/20 p-3 rounded-2xl mb-4'>
				...
				<div className='space-y-3'>
					{(currentUser.freeSpins || 0) > 0 ? (
						<Button
							onClick={handleSpin}
							disabled={isSpinning}
							className='w-full h-12 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold rounded-xl disabled:opacity-50 disabled:cursor-not-allowed'
						>
							{isSpinning ? (
								<>
					...
					<Button
						...
						variant='outline'
						disabled={isSpinning}
						className='w-full h-10 bg-transparent border-2 border-gray-500/50 text-gray-300 hover:bg-gray-600/20 hover:border-gray-400 rounded-xl disabled:opacity-50'
					>
